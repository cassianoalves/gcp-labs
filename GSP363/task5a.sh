...
# Políticas Existentes das Etapas Anteriores
...
echo "=== 2. Criando a Nova Política de Erro (AM-BuildErrorResponse) ==="
cat <<EOF > ${PROXY_NAME}/apiproxy/policies/AM-BuildErrorResponse.xml
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
echo "=== 4. Criando o TargetEndpoint com a seção FaultRules (default.xml) ==="
cat <<EOF > ${PROXY_NAME}/apiproxy/targets/default.xml
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<TargetEndpoint name="default">
    <Description/>
    <FaultRules>
        <FaultRule name="InvalidTargetLanguage">
            <Step>
...
echo "=== 5. Configurando o Manifesto Principal para a Revisão 4 ==="
cat <<EOF > ${PROXY_NAME}/apiproxy/${PROXY_NAME}.xml
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<APIProxy revision="4" name="${PROXY_NAME}">
    ...
    <Policies>
        ...
        <Policy>AM-BuildErrorResponse</Policy>
    </Policies>
    <ProxyEndpoints>
...
echo "=== 6. Compactando o pacote do proxy ==="
cd ${PROXY_NAME}
...
echo "=== 7. Enviando a Revisão 4 para o Apigee (Import) ==="
...
rm -rf ${PROXY_NAME} ${PROXY_NAME}.zip
echo -e "\n\nProxy implantado com sucesso! Monitoramento de FaultRules ativo para erros 400." auditoria de logs de tradução ativa na Revisão 3!"